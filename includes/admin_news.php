 ...
 */
	...
	if (isset($_GET['page']) == "") { 
	$result = mysql_query("SELECT * FROM middlenews LIMIT 0, 20");
	} else {
		if ($_GET['page'] == 0) {
		$result = mysql_query("SELECT * FROM middlenews LIMIT 0, 20");
		} else {
				$x = $_GET['page'] * 20;
		$y = $_GET['page'] * 40;
		$result = mysql_query("SELECT * FROM middlenews LIMIT ".x.", ".y."");
		}
	}
		...
		<tbody><tr>';
				...
				<td class="windowbg2">
					<span class="smalltext">ThreadId</span>


				</td>
				<td class="windowbg" valign="middle" align="center" style="width: 25%"><span class="smalltext">Using thread</span></td>
	...
	<table border="0" width="100%" cellspacing="1"  cellpadding="5" style="margin-top: 1px; background: #E2F4FF;">
		<tbody><tr>';
				...
				<td class="windowbg2">
					<b><a href="http://'.scriptUrl.'/threads/'.$row['threadid'].'" name="b2">'.$row['threadid'].'</a> </b> - (<a href="http://'.scriptUrl.'/admin.php?p=newssettings&id='.$row['id'].'">Modify</a>)
				</td>
...
';
  ...
  if(($page+1) <= ($results/20)) {
	...
	echo '<br /><a href="http://'.scriptUrl.'/admin.php?p=newnews">Add News</a>';
